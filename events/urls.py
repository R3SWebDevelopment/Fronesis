...
urlpatterns = [
    ...
    url(r'^my_events/$', permission_required('events.edit_event')(MyEventsView.as_view()), name='my_events'),
    url(r'^my_events/create/$', permission_required('events.create_event')(CreateEventView.as_view()),
        {'mode': 'create'}, name='my_events_create'),
    url(r'^my_events/(?P<event_uuid>[0-9A-Fa-f-]+)/edit/$',
        permission_required('events.create_event')(EventView.as_view()), {'mode': 'update'}, name='my_events_update'),
    url(r'^my_events/(?P<event_uuid>[0-9A-Fa-f-]+)/tickets_sales_report/$',
        permission_required('events.edit_event')(EventTicketSalesReport.as_view()),
